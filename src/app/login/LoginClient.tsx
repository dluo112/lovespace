...
export default function LoginClient({ initialProfiles, initialSettings }: { initialProfiles: any, initialSettings: any }) {
  const [selectedRole, setSelectedRole] = useState<'boy' | 'girl' | null>(null);
  const [state, formAction, isPending] = useActionState(login, initialState);
  const [profiles, setProfiles] = useState(initialProfiles);
  const [enableGuest, setEnableGuest] = useState(initialSettings?.success ? initialSettings.data.enableGuestAccess : true);
