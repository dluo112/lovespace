...
import { Save, LogOut, Camera, ChevronRight, Trash2, Plus, Calendar, X, Lock, Hourglass, Mail, Image as ImageIcon, Gift, Sparkles } from 'lucide-react';
import { updateProfile, updateSystemSettings } from '@/app/actions/settings';
import { createAnniversary, deleteAnniversary, createCapsule, deleteCapsule, getAnniversaries, getCapsules } from '@/app/actions/admin';
...
export default function ProfileClient({ 
  ...
  // Form State
  const [avatar, setAvatar] = useState(initialProfile?.data?.avatar || "https://api.dicebear.com/9.x/adventurer/svg?seed=Felix");
  const [name, setName] = useState(initialProfile?.data?.name || '');
  const [password, setPassword] = useState('');
  
  const [startDate, setStartDate] = useState(
    initialSettings?.data?.startDate 
      ? new Date(initialSettings.data.startDate).toISOString().split('T')[0] 
      : ''
  );
  const [enableGuestAccess, setEnableGuestAccess] = useState(initialSettings?.data?.enableGuestAccess ?? true);
  
  // Anniversary State
  const [anniversaries, setAnniversaries] = useState<any[]>(initialAnniversaries || []);
  ...
  // Capsule State
  const [capsules, setCapsules] = useState<any[]>(initialCapsules || []);
  ...
  const [currentUserId] = useState<string>(initialProfile?.data?.id || '');

  // Ritual States
  const [sealing, setSealing] = useState(false);
  const [openingState, setOpeningState] = useState<'locked' | 'gift' | 'open'>('locked');

  const [initialName, setInitialName] = useState(initialProfile?.data?.name || '');
    ...
    <div className="min-h-screen bg-slate-50 pb-safe">
      ...
      <main className="px-4 py-6 max-w-md mx-auto space-y-8">
        ...
        {viewingCapsule && (
                                ...
                                {viewingCapsule.images && viewingCapsule.images.length > 0 && (
                                    <div className="space-y-3 mb-6">
                                        {viewingCapsule.images.map((img: string, idx: number) => (
                                            <img key={idx} src={img} alt="回忆图片" className="w-full rounded-xl shadow-sm" />
                                        ))}
                                    </div>
                                )}
                            </div>
            ...
            <h2 className="text-2xl font-bold text-slate-800">{name || '用户'}</h2>
        </div>
