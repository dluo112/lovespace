import { getMemory, createComment, deleteMemory, updateMemory } from '@/app/actions/admin';
import { notFound, redirect } from 'next/navigation';
import Link from 'next/link';
import { ChevronLeft, MapPin, Clock, Send, MoreVertical, Trash2, Edit2, X, Image as ImageIcon } from 'lucide-react';
import Image from 'next/image';
import CommentForm from './CommentForm';
import EditMemoryForm from './EditMemoryForm';

export default async function MemoryDetailPage({ params }: { params: { id: string } }) {
  const { id } = await params; // Await params as required in newer Next.js
  ...
  return (
    <div className="min-h-screen pb-safe">
      ...
      <header className="sticky top-0 z-20 bg-slate-50/80 backdrop-blur-md px-4 py-3 flex items-center gap-3 border-b border-slate-200/50">
        <Link href="/admin/timeline" className="text-slate-500 hover:text-slate-800 transition-colors">
          <ChevronLeft size={24} />
        </Link>
        <h1 className="text-lg font-semibold text-slate-800 tracking-tight flex-1">回忆详情</h1>
        
        {/* Actions Dropdown (Simplified as direct buttons for now, or client component needed for dropdown) */}
        {/* We'll use a client component wrapper for edit/delete to handle state */}
      ...
      <main className="px-4 py-6 max-w-md mx-auto space-y-6">
        ...
        <div className="bg-white rounded-2xl p-5 shadow-sm border border-slate-100">
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            {memory.isPublic && (
              <span className="text-[10px] bg-brand-pink/10 text-brand-pink px-2 py-0.5 rounded-full font-medium">
                公开
              </span>
            )}
          </div>
